...
# OverlapPeaksWithChippeakanno <- function(peaks.list, max.gap, min.overlap, ignore.strand, connected.peaks) {
#   require(ChIPpeakAnno)
#  ## findOverlapsOfPeaks 
...
# peak.list= peaks
#  
# annotation.data=gtf.gr 
# 
# feature.type= "TSS"
# output=  "both"
# max.gap=0L
# peak.loc.for.distance="middle" 
...
Gtf2GrangeForChipPeakAnno <- function(gtf.data) {
  ...
  # head(gtf.bed.cpa)
  ##print("grange")
  gtf.gr = GRanges(seqnames = Rle(gtf.bed.cpa.u$chr), ranges=IRanges(start=gtf.bed.cpa.u$start, end=gtf.bed.cpa.u$end, names=gtf.bed.cpa.u$gene_id), strand = gtf.bed.cpa.u$strand, name = as.character(gtf.bed.cpa.u$gene_id))
